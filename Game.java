...
class Game {
	...
	public void update() {
		...
		if ((int) MouseInfo.getPointerInfo().getLocation().getX() > 100 && (int) MouseInfo.getPointerInfo().getLocation().getX() < 200 && (int) MouseInfo.getPointerInfo().getLocation().getY() > 100 && (int) MouseInfo.getPointerInfo().getLocation().getY() < 200) {
		...
		if (gameStart) {
			/*
			 * Player player = new
			 * Archer((int)Toolkit.getDefaultToolkit().getScreenSize().getWidth(
			 * )/2,(int)Toolkit.getDefaultToolkit().getScreenSize().getHeight()/
			 * 2,100,100,0); Enemy enemy1 = new Enemy(75,40,50,60);
			 * ArrayList<Enemy> enemies = new ArrayList<Enemy>();
			 * ArrayList<Wall> obstacles = new ArrayList<Wall>(); int
			 * reloadCount = 0; enemies.add(enemy1); enemies.add(new
			 * Enemy(75,40,300,300)); Display draw = new Display(this);
			 */
			if (draw == null) {
				return;
			}
			...
			for (int i = 0; i < obstacles.size(); i++) {
				...
				for (int j = 0; j < enemies.size(); j++) {
					...
					if (obstacles.get(i).getHitbox().intersects(enemies.get(j).getHitbox())) {
						...
						// System.out.println(displaceModifier[0] + " " +
						// displaceModifier[1]);
					}
				}
			...
			for (int i = 0; i < player.getProjectiles().size(); i++) {
				...
				for (int j = 0; j < enemies.size(); j++) {
					if ((enemies.get(j).getHitbox().intersects(player.getProjectiles().get(i).getHitbox())) && ((player.getProjectiles().get(i) instanceof FireExplosion) || (player.getProjectiles().get(i) instanceof FireDagger) || (player.getProjectiles().get(i) instanceof FireSpell) || (player.getProjectiles().get(i) instanceof FireTrapExplosion))) {// Projectile
																																																																																									// damages
																																																																																									// the
																																																																																									// enemy
				...
				if ((player.getProjectiles().get(i).getLifeTime() > 50) && ((player.getProjectiles().get(i) instanceof FireExplosion) || (player.getProjectiles().get(i) instanceof IceExplosion) || (player.getProjectiles().get(i) instanceof WaterDash) || (player.getProjectiles().get(i) instanceof EarthStun) || (player.getProjectiles().get(i) instanceof FireSpell) || (player.getProjectiles().get(i) instanceof IceSpell) || (player.getProjectiles().get(i) instanceof FireTrapExplosion) || (player.getProjectiles().get(i) instanceof IceTrapExplosion) || (player.getProjectiles().get(i) instanceof EarthTrapExplosion) || (player.getProjectiles().get(i) instanceof AirTrapExplosion))) {// Controls
																																																																																																																																																																											// how
																																																																																																																																																																											// long
																																																																																																																																																																											// an
																																																																																																																																																																											// aoe
			...
			for (int i = 0; i < enemies.size(); i++) {
				if (enemies.get(i) instanceof ProjectileE) {
					for (int j = 0; j < ((ProjectileE) (enemies.get(i))).getProjectiles().size(); j++) {
						...
						// if
						// ((((ProjectileE)(enemies.get(i))).getProjectiles().get(j).getTargetX()==((ProjectileE)(enemies.get(i))).getProjectiles().get(j).getX())
						// &&
						// (((ProjectileE)(enemies.get(i))).getProjectiles().get(j).getTargetY()==((ProjectileE)(enemies.get(i))).getProjectiles().get(j).getY())){
						// playerColl = true;
						// }
			...
			if (Math.random() < 0.01) {
				// enemies.add(new Enemy(5,40, (int)(Math.random()*400),
